...
class SQLServer extends InterConnection {
  ...
  async select (table, argument, where) {
    let query = ''
    if (where !== undefined) {
      query = 'SELECT ' + argument + ' FROM ' + table + ' WHERE ' + formatObject(where) + ';'
    } else {
  ...
  async selectTop (table, records) {
    const query = 'SELECT TOP ' + records + ' * FROM ' + table + ';'
    try {
      const result = await this.connection.getConfiguration().sql(query).execute()
      return result
    } catch (error) {
  ...
  async delete (argument, table, condition) {
    let query = ''
    if (argument === undefined || argument === '' || argument === null) {
  ...
  async orderBy (tables, argument, sortObject) {
    let query = ''
