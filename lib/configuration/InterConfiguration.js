...
import { selectDb, DB_NAMES } from '../helpers/strategy'
...
class InterConfiguration {
  ...
  */
  static getInstance (database, user, password, options) {
    ...
    const TYPE = options !== undefined ? options.type || DB_NAMES[0] : DB_NAMES[0]
    let DbStrategy = selectDb(TYPE, '../configuration')
    return new DbStrategy(database, user, password, options)
  }
